...
#    read
    ...
    echo   $4/$2:$3/$2 \\ >> $PROPRIETARY_BLOBS_LIST
        ...
        if [ "$EXTRACT_BLUEDROID_FILES" == 1 ]; then
            echo
            echo "############################################################"
            echo "Can't find necessary file(s) of Bluedroid in the backup-${DEVICE} folder."
            echo "Please update the system image for supporting Bluedroid (Bug-986314), "
            echo "so that the needed binary files can be extracted from your flatfish device."
            echo "############################################################"
            echo
        fi
        exit -1
...
EXTRACT_BLUEDROID_FILES=1

BLUEDROID_HW="
    bluetooth.default.so
    "
copy_files "$BLUEDROID_HW" "system/lib/hw" "allwinner/$DEVICE/proprietary"

BLUEDROID_LIBS="
    libbt-hci.so
    libbt-utils.so
    "
copy_files "$BLUEDROID_LIBS" "system/lib" "allwinner/$DEVICE/proprietary"
...
BLUEDROID_CONF="
    ...
    bt_vendor.conf
    "
copy_files "$BLUEDROID_CONF" "system/etc/bluetooth" "allwinner/$DEVICE/proprietary"

BLUEDROID_VENDOR_LIBS="
    libbt-vendor.so
    "
copy_files "$BLUEDROID_VENDOR_LIBS" "system/vendor/lib" "allwinner/$DEVICE/proprietary"

BLUEDROID_VENDOR_MODULES="
    BCM4330B1.hcd
    "
copy_files "$BLUEDROID_VENDOR_MODULES" "system/vendor/modules" "allwinner/$DEVICE/proprietary"

EXTRACT_BLUEDROID_FILES=""
